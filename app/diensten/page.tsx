...
const diensten = [
  {
    ...
    body: "Helps merken hun unieke stem vinden en versterken — van websiteteksten en whitepapers tot direct response en brand storytelling.",
    items: ["Webcontent & SEO-geoptimaliseerde artikelen", "Brand storytelling & missie/visie statements", "Direct response & salesteksten", "AI-prompts die werken voor uw bedrijf"],
  },
  {
    tag: "02",
    ...
    items: ["Grondige eindredactie", "Stijl- en taalcorrectie", "Proeflezen voor publicatie", "Redactie van bestaande webteksten"],
  },
  {
    tag: "03",
    title: "Quickscan voor uw website",
    ...
    items: ["Leesbaarheid & structuur", "Doelgroepgerichtheid", "Online vindbaarheid (SEO)", "Kansen en verbeterpunten"],
  },
  {
    tag: "04",
      ...
      <section className="bg-[var(--color-surface-container-low)] py-24">
        <div className="max-w-6xl mx-auto px-6 space-y-4">
          ...
          <p className="text-lg text-[var(--color-on-surface-variant)] max-w-xl leading-relaxed">
            Van een gratis quickscan tot een volledige contentstrategie — altijd met uw eigen stem en authenticiteit als vertrekpunt.
          </p>
        </div>
      </section>
      ...
      <section className="bg-[var(--color-surface)] py-24">
        <div className="max-w-6xl mx-auto px-6 space-y-6">
          {diensten.map(({ tag, title, subtitle, body, items }) => (
            <div
              key={tag}
              className="bg-[var(--color-surface-container-lowest)] rounded-[var(--radius-brand)] p-10 grid md:grid-cols-[1fr_2fr] gap-10 shadow-[0_4px_24px_rgba(0,32,30,0.04)]"
            >
              <div className="space-y-3">
                <p className="font-[var(--font-label)] text-xs font-bold uppercase tracking-[0.3em] text-[var(--color-outline)]">
                  {tag}
                </p>
                <h2 className="font-[var(--font-headline)] text-2xl font-bold italic text-[var(--color-on-surface)]">
                  {title}
                </h2>
                <span className="inline-block text-xs font-bold uppercase tracking-widest text-[var(--color-secondary)] font-[var(--font-label)]">
                  {subtitle}
                </span>
              </div>
      ...
      <section className="bg-[var(--color-surface-container-low)] py-24">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          <div className="space-y-2">
            ...
            <p className="text-[var(--color-on-surface-variant)]">We zijn pas klaar als u tevreden bent.</p>
          </div>
          <Link
            href="/contact"
            className="inline-flex items-center bg-[var(--color-secondary)] text-[var(--color-on-secondary)] text-xs font-bold uppercase tracking-widest px-8 py-4 rounded-[var(--radius-brand)] hover:bg-[var(--color-secondary-container)] transition-colors font-[var(--font-label)] flex-shrink-0"
          >
            Start een project
          </Link>
        </div>
